<?php
    ...
    function getFullname(){
        global $db;
        $sql = "SELECT fullname from users where username ='".$_SESSION["user"]."' and pass='".$_SESSION["pass"]."';";
        $result = $db->query($sql)->fetch_assoc();
    ...
    function getInfoCustomer(){
		...
		$sql = "SELECT c.fullname, c.address, c.phone from customer as c, users as u where c.fullname = '".getFullname()."'";
		$info = $db->query($sql)->fetch_all();
    }
    ...
    function addToOrder(){
    	global $db;
		$now = getdate();
		$sql2 = "INSERT INTO orderd(idCart, nameCus, phone, address, datebuy, total) VALUES()";
		$db->query($sql2);
    }
?>